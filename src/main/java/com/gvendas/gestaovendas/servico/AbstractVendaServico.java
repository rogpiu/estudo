...
public abstract class AbstractVendaServico {
	...
	protected VendaResponseDTO criandoVendaResponseDTO(Venda venda, List<ItemVenda> itensVendaList) {
		List<ItemVendaResponseDTO> itensVendaDTO = itensVendaList.stream()
				.map(this::criandoItemVendaResponseDTO).collect(Collectors.toList());
		return new VendaResponseDTO(venda.getCodigo(), venda.getData(), itensVendaDTO);
	}
	...
	protected ClienteVendaResponseDTO retornaClienteVendaResponseDto(Venda venda, List<ItemVenda> itemVenda) {
		return new ClienteVendaResponseDTO(venda.getCliente().getNome(), Arrays
				.asList(criandoVendaResponseDTO(venda, itemVenda)));
